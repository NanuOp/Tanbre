...
String readDecryptedFile(FS &fs, String filepath) {
    ...
    while (cyphertextFile.available()) {
        ...
        if (line.startsWith("Filetype:") && !line.endsWith("Bruce Encrypted File") && !line.endsWith("Tanbre Encrypted File")) unsupported_params = true;
...
String encryptString(String &plaintext, const String &password_str) {
    ...
    String out = "Filetype: Tanbre Encrypted File\nVersion: 1\n";
#else
