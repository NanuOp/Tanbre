...
public:
    ...
    const char *filepath = "/tanbre.conf";
#else
    ...
    // Wifi
#if defined(TANBRE_WATCH)
    Credential webUI = {"admin", "tanbre"};
    std::vector<String> webUISessions = {}; // FIFO queue of session tokens
    WiFiCredential wifiAp = {"TanbreNet", "tanbrenet"};
#else
...
#if defined(TANBRE_WATCH)
        {"Tanbre AP",   "WIFI:T:WPA;S:TanbreNet;P:tanbrenet;;"},
        {"Tanbre Wiki", "https://github.com/pr3y/Bruce/wiki"},
        {"Tanbre Site", "https://bruce.computer"            },
#else
