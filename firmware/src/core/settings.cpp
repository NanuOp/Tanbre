...
#if defined(TANBRE_WATCH)
    const bool isDefault = bruceConfig.wifiAp.ssid == "TanbreNet";

    options = {
        {"Default (TanbreNet)",
         [=]() { bruceConfig.setWifiApCreds("TanbreNet", bruceConfig.wifiAp.pwd); },
         isDefault                                                                            },
        {"Custom",
         [=]() {
...
#if defined(TANBRE_WATCH)
    const bool isDefault = bruceConfig.wifiAp.pwd == "tanbrenet";

    options = {
        {"Default (tanbrenet)",
         [=]() { bruceConfig.setWifiApCreds(bruceConfig.wifiAp.ssid, "tanbrenet"); },
         isDefault                                                                             },
        {"Custom",
         [=]() {
...
#if defined(TANBRE_WATCH)
    return fs->exists("/TanbreJS/Tools/App Store.js") || fs->exists("/BruceJS/Tools/App Store.js");
#else
...
void installAppStoreJS() {
...
#if defined(TANBRE_WATCH)
    const char* jsFolder = "/TanbreJS";
    const char* toolsFolder = "/TanbreJS/Tools";
    const char* appStorePath = "/TanbreJS/Tools/App Store.js";
#else
    const char* jsFolder = "/BruceJS";
    const char* toolsFolder = "/BruceJS/Tools";
    const char* appStorePath = "/BruceJS/Tools/App Store.js";
#endif

    if (!fs->exists(jsFolder)) {
        if (!fs->mkdir(jsFolder)) {
            displayWarning(String("Failed to create ") + jsFolder + " directory", true);
            return;
        }
    }

    if (!fs->exists(toolsFolder)) {
        if (!fs->mkdir(toolsFolder)) {
            displayWarning(String("Failed to create ") + toolsFolder + " directory", true);
            return;
        }
    }
    ...
    File file = fs->open(appStorePath, FILE_WRITE);
    if (!file) {
