...
#if defined(TANBRE_WATCH)
        int idx = c.indexOf("TANBRESESSION=");
        if (idx != -1) {
            int start = idx + 14;
#else
...
void drawWebUiScreen(bool mode_ap) {
...
#if defined(TANBRE_WATCH)
        tft.print("Net: TanbreNet/tanbrenet");
#else
    ...
    tft.setCursor(padX, currentY);
#if defined(TANBRE_WATCH)
    tft.print("Url: http://tanbre.local");
#else
...
void serveWebUIFile(
    ...
    if (setupSdCard()) {
#if defined(TANBRE_WATCH)
        if (SD.exists("/TanbreWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/TanbreWebUI/" + filename)) {
#else
    ...
    if (fs) {
#if defined(TANBRE_WATCH)
        response = request->beginResponse(*fs, "/TanbreWebUI/" + filename, contentType);
#else
...
#if defined(TANBRE_WATCH)
                response->addHeader("Set-Cookie", "TANBRESESSION=" + token + "; Path=/; HttpOnly");
#else
...
#if defined(TANBRE_WATCH)
        response->addHeader("Set-Cookie", "TANBRESESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
#else
