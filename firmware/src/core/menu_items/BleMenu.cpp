...
#include "modules/ble/blescan.h"
#include "modules/ble/airtag_spoofer.h"
#include "modules/ble/ble_spammer.h"
#include "modules/ble/ble_spoofer.h"
#include "modules/ble/ble_inspector.h"
#include "modules/ble/drone_spoofer.h"
#include "modules/ble/nyanbox_advertiser.h"
#include "modules/ble/pwnagotchi_spam.h"
#endif
#include <globals.h>
...
void BleMenu::optionsMenu() {
    ...
    options.push_back({"iBeacon", [=]() {
                           ...
                           ibeacon("Tanbre", "e4c159a0-8c82-11e6-bdf4-0800200c9a66", 0x004C);
#else
    ...
    options.push_back({"BLE Scanner", runBlescan});
    options.push_back({"Apple Spoofer", runAirtagSpoofer});
    options.push_back({"BLE Spammer", runBleSpammer});
    options.push_back({"BLE Spoofer", runBleSpoofer});
    options.push_back({"BLE Inspector", runBleInspector});
    options.push_back({"Drone Spoofer", runDroneSpoofer});
    options.push_back({"nyanBOX Adv", runNyanboxAdvertiser});
    options.push_back({"Pwnagotchi Spam", runPwnagotchiSpam});
#endif
    addOptionToMainMenu();
