    ...
    while (1) {
        ...
        if (menuType == MENU_TYPE_MAIN) {
            ...
            if (clock_set) {
                ...
                int current_min = -1;
#if defined(HAS_RTC)
                current_min = _rtc.getTimeStruct().tm_min;
#else
                current_min = rtc.getTimeStruct().tm_min;
#endif
        ...
        if (redraw) {
            ...
            Serial.printf("[MENU] Menu: %s | Active Item: %s\n", subText ? subText : "Main/None", options[index].label.c_str());
            // update the hovered
            ...
            if (menuType == MENU_TYPE_MAIN) {
                drawStatusBar(); // Draw status bar on top of options
            }
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    // Center Y is 45. FM size is 8px tall, so Y starts at 45 - 4 = 41.
    int y_sel = 41;
    tft.drawCentreString(options[index].label, tftWidth / 2, y_sel, 1);
#else
...
void drawStatusBar() {
    ...
    if (clock_set) {
        ...
        setTftDisplay(12, 12, bruceConfig.priColor, 1, bruceConfig.bgColor);
#if defined(TANBRE_WATCH)
        tft.print("TANBRE " + String(BRUCE_VERSION));
#else
...
void drawBatteryStatus(uint8_t bat) {
    ...
    // Clear only a small right area (from tftWidth - 42 to tftWidth - 8)
    tft.fillRect(tftWidth - 42, 12, 34, 10, bruceConfig.bgColor);
    if (charging) {
        tft.drawRightString("CHG", tftWidth - 12, 12, 1);
    } else {
        tft.drawRightString(String(bat) + "%", tftWidth - 12, 12, 1);
    }
#else
