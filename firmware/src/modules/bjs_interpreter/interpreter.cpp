...
void interpreterHandler(void *pvParameters) {
    ...
    JS_SetPropertyStr(ctx, global, "TANBRE_VERSION", JS_NewString(ctx, BRUCE_VERSION));
    JS_SetPropertyStr(ctx, global, "TANBRE_PRICOLOR", JS_NewInt32(ctx, bruceConfig.priColor));
    JS_SetPropertyStr(ctx, global, "TANBRE_SECCOLOR", JS_NewInt32(ctx, bruceConfig.secColor));
    JS_SetPropertyStr(ctx, global, "TANBRE_BGCOLOR", JS_NewInt32(ctx, bruceConfig.bgColor));
...
String getScriptsFolder(FS *&fs) {
    String folder;
#if defined(TANBRE_WATCH)
    String possibleFolders[] = {"/scripts", "/TanbreScripts", "/TanbreJS", "/BruceScripts", "/BruceJS"};
#else
