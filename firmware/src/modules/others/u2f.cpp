...
void drawU2fStatusScreen() {
    ...
    tft.setCursor(6, (tftHeight <= 80) ? 5 : 8);
    tft.print("USB U2F");

    tft.setTextSize(FP);
    tft.setCursor(6, (tftHeight <= 80) ? 22 : 36);
    tft.print("Ready for reg/login");
    tft.setCursor(6, (tftHeight <= 80) ? 34 : 50);
    tft.print("Press center when prompted");
    tft.setCursor(6, (tftHeight <= 80) ? 46 : 64);
    tft.print("ESC: Back");
}

void updateU2fRuntimeInfo(const U2fHidDevice &device) {
    tft.fillRect(0, (tftHeight <= 80) ? 58 : 84, tftWidth, tftHeight - ((tftHeight <= 80) ? 58 : 84), bruceConfig.bgColor);
    tft.setTextSize(FM);
    tft.setCursor(6, (tftHeight <= 80) ? 58 : 94);
