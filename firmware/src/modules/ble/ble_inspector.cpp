...
std::vector<InspectorBLEDevice> bleInspectorDevices;
...
static void buildDetailRows(const InspectorBLEDevice& dev) {
    numRows = 0;
...
static void store_raw(InspectorBLEDevice& dev, esp_ble_gap_cb_param_t* sr) {
...
static void extract_name(esp_ble_gap_cb_param_t* sr, InspectorBLEDevice& dev) {
    ...
    for (auto& dev : bleInspectorDevices) {
        ...
        if (inspView != BLEINSP_LOCATE)
            std::sort(bleInspectorDevices.begin(), bleInspectorDevices.end(),
                      [](const InspectorBLEDevice& a, const InspectorBLEDevice& b)
                      { return a.rssi > b.rssi; });
        return;
    }

    InspectorBLEDevice dev = {};
...
static void drawLocate() {
    ...
    const InspectorBLEDevice& dev = bleInspectorDevices[currentIndex];
    char buf[32];
...
void bleInspectorLoop() {
    ...
    if (bleInspectorDevices.empty()) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            const InspectorBLEDevice& d = bleInspectorDevices[idx];
...
void runBleInspector() {
    runNyanModule(bleInspectorSetup, bleInspectorLoop, nullptr);
}
