...
void startSessionLog(ClientProtocol protocol) {
    withClientLock([&]() {
        ...
        if (!SD.exists("/Tanbre")) SD.mkdir("/Tanbre");
        if (!SD.exists("/Tanbre/Terminal")) SD.mkdir("/Tanbre/Terminal");

        String basePath = "/Tanbre/Terminal/" + String(getProtocolName(protocol)) + "-" +
...
#if defined(TANBRE_WATCH)
            String header = "=== Tanbre Terminal Session ===\n";
#else
