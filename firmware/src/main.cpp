...
void begin_storage() {
    ...
    if (LittleFS.exists("/brucePins.conf")) {
...
void boot_screen() {
    ...
    tft.setTextSize(FG);
    tft.drawCentreString("TANBRE", tftWidth / 2, 16, 1);
    tft.setTextSize(FP);
    tft.drawCentreString("BY G.SHIVPRASAD", tftWidth / 2, 38, 1);
#else
    tft.setTextSize(FM);
    tft.drawCentreString("TANBRE", tftWidth / 2, 10, 1);
    tft.setTextSize(FP);
...
void boot_screen_anim() {
    ...
    tft.drawPixel(0, 0, 0);       // Forces back communication with TFT, to avoid ghosting
                                  // Start image loop
    while (millis() < i + 2000) { // boot image lasts for 2 secs
...
#if !defined(LITE_VERSION) && !defined(TANBRE_WATCH)
        if (!boot_img && (millis() - i > 200) && (millis() - i) < 400)
            tft.drawRect(2 * tftWidth / 3, tftHeight / 2, 2, 2, bruceConfig.priColor);
        if (!boot_img && (millis() - i > 400) && (millis() - i) < 600)
            tft.fillRect(0, 45, tftWidth, tftHeight - 45, bruceConfig.bgColor);
        if (!boot_img && (millis() - i > 600) && (millis() - i) < 1000)
            tft.drawXBitmap(
                2 * tftWidth / 3 - 30,
                5 + tftHeight / 2,
                bruce_small_bits,
        ...
        if (!boot_img && (millis() - i > 1000) && (millis() - i) < 1500) tft.fillScreen(bruceConfig.bgColor);
        if (!boot_img && (millis() - i > 1500))
            tft.drawXBitmap(
                (tftWidth - 238) / 2,
                (tftHeight - 133) / 2,
                bits,
    ...
    // Clear splashscreen
    tft.fillScreen(bruceConfig.bgColor);
    delay(50); // Reduced from default delay
}
...
void setup() {
    ...
    if (!bruceConfig.instantBoot) {
        Serial.println("[DEBUG] Starting boot_screen_anim...");
        boot_screen_anim();
        Serial.println("[DEBUG] finished boot_screen_anim.");
        startup_sound();
    }
...
#if defined(HAS_SCREEN)
void loop() {
    Serial.println("[DEBUG] loop() iteration started...");
